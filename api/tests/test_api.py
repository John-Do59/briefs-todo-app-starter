...
def test_create_todo_with_subtask(client):
    parent = client.post("/todos", json={"title": "Parent task"}).json()
    subtask = client.post(
        "/todos",
        json={"title": "Subtask", "parent_id": parent["id"]},
    ).json()
    assert subtask["parent_id"] == parent["id"]
    parent_response = client.get(f"/todos/{parent['id']}").json()
...
# Dependency tests
def test_add_todo_dependency(client):
    todo1 = client.post("/todos", json={"title": "Do first"}).json()
    todo2 = client.post("/todos", json={"title": "Do second"}).json()

    response = client.post(f"/todos/{todo2['id']}/dependencies/{todo1['id']}")
    assert response.status_code == 201

    todo2_response = client.get(f"/todos/{todo2['id']}").json()
    assert todo1["id"] in todo2_response["depends_on"]


def test_add_self_dependency(client):
    todo = client.post("/todos", json={"title": "Self"}).json()
    response = client.post(f"/todos/{todo['id']}/dependencies/{todo['id']}")
    assert response.status_code == 400


def test_remove_todo_dependency(client):
    todo1 = client.post("/todos", json={"title": "1"}).json()
    todo2 = client.post("/todos", json={"title": "2"}).json()
    client.post(f"/todos/{todo2['id']}/dependencies/{todo1['id']}")

    response = client.delete(f"/todos/{todo2['id']}/dependencies/{todo1['id']}")
    assert response.status_code == 204
